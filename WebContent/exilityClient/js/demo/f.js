    ...
    */
    ...
    function Plot(container, data, opts, chartType) {
        ...
        */
        ...
        var chartData; //for all other charts
        ...
        */
        function calculateSpacing() {
            ...
            for (var i = 0; i < yaxis.ticks.length; ++i) {
                ...
                if (yaxis.ticks[i].label.length > 50) {
                    yaxis.ticks[i].label = yaxis.ticks[i].label.substring(0, 47) + '...';
                }
                //end of addition
            }
            ...
            */
            ...
            labelMaxWidth = getlength(max_label) * 9 + 15; //temp patch
            labelMaxHeight = 17; //temp patch
        ...
        */
        function drawGrid() {
            ...
            */
            if ('BAR' == chartType || 'STACKED' == chartType) {
            ...
            function plotBars(data, barWidth, offset, fill, label) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    bar.barCordinates.push({ leftPx: xleft,
                        ...
                        sequence: sequence,
                        idx: data[i][2],
                        helpText: data[i][3] || ('help text Bar-' + chartData.bars.length + ' bar ' + i)
                    });
                    ...
                    */
                    ...
                    if (fill) {
                        ...
                        ctx.fill();
                        //drawTextOnBars(left, right, top, bottom, i, x, y, currentindex, serieslength); //added by vinay for labels on stacked chart
                    }
        ...
        function drawBarLabels() {
            ...
            for (var barIdx = 0; barIdx < bars.length; barIdx++) {
                ...
                for (var individualBarIdx = 0; individualBarIdx < individualBar.length; individualBarIdx++) {
                    var bar = individualBar[individualBarIdx];
                    //PM.debug('\n chartType '+chartType+' left '+bar.left+' right '+bar.right+'  top '+bar.top+' bottom '+bar.bottom +' sequence '+bar.sequence);
                    ...
                    if ('STACKED' == chartType) {
                        ...
                        var midPx = bar.leftPx + (bar.rightPx - bar.leftPx) / 4;

                        if ((bar.rightPx - bar.leftPx) < 15)
                            midPx = bar.leftPx;

                        var labelVal = Math.round(top - previousTop);
                        var labelLen = getlength(labelVal);

                        if ((labelLen * 7) < (bar.bottomPx - bar.topPx)) {
                            ctx.save();
                            ctx.translate(midPx, midPy);
                            ctx.rotate(-Math.PI / 2);
                            ctx.textAlign = "center";
                            ctx.fillStyle = 'white';
                            ctx.fillText(labelVal, 0, 10);
                            ctx.restore();
                        }
                        ...
                        if (barIdx == nbrStacks - 1)/* write the count on the topest stack */
                        {
                            ...
                            ctx.fillText(top, midPx, bar.topPx - 5);
                        }
                    }
                        ...
                        if ((bar.bottomPx - bar.topPx) >= 15) {
                            var labelVal = Math.round(right - previousRight);
                            var labelLen = getlength(labelVal);

                            if ((labelLen * 5) < (bar.rightPx - bar.leftPx)) {
                                ctx.fillStyle = 'white';
                                ctx.fillText(labelVal, midPx, midPy + 5); /* write the count on each stack */
                            }
                        }
        ...
        */
        ...
        function mouseMoveOnBar(event, eventName) {
            ...
            if (barFocussed) {
                highlightlegend(currentBarIdx + 1);
                ...
                octx.closePath();
                var clr = parseColor(bars[currentBarIdx].color);
                clr.scale(1.1, 1.1, 1.1);
                octx.fillStyle = clr.toString();
                octx.fill();

                var midPy = bottom - (bottom - top) / 2;
                var midPx = left + (right - left) / 4;

                if ((right - left) < 15)
                    midPx = left;

                if ('STACKED' == chartType || 'BAR' == chartType) {
                    var labelVal = Math.round(barFocussed.val);
                    var labelLen = getlength(labelVal);

                    if ((labelLen * 7) < (bottom - top)) {
                        octx.save();
                        octx.translate(midPx, midPy);
                        octx.rotate(-Math.PI / 2);
                        octx.textAlign = "center";
                        octx.fillStyle = 'white';
                        octx.fillText(labelVal, 0, 10);
                        octx.restore();
                    }
                }
                ...
                if ('HORIZONTALSTACKED' == chartType) {
                    if ((bottom - top) >= 15) {
                        octx.fillStyle = 'white';
                        octx.fillText(barFocussed.val, midPx, midPy + 5); /* write the count on each stack */
                    }
                }
        ...
        function mouseMoveHandler(event, eventName) {
            ...
            * */
            ...
            if ('PIE' === chartType || 'SUNBURST' === chartType) {
                ...
                if (eventName == 'mouseClick' && 'PIE' == chartType) {
                    if (rowIdx != null) {
                        if (pieData.lastIdx == null || pieData.lastIdx != rowIdx) {
                            pos.rowIdx = rowIdx + 1;
                            target.fire('flotr:onclick', [event, pos]);
                        }
                ...
                if ('PIE' === chartType && window.highlightRow) {
                    if (rowIdx != null) {
                        if (pieData.lastIdx == null || pieData.lastIdx != rowIdx) {
                            var color = pieData.colors[rowIdx];
                            highlightRow(rowIdx + 1, color);
                            highlightlegend(rowIdx + 1);
                        }
                    }
                    else if (pieData.lastIdx != null) {
                        highlightRow(-1, null);
        ...
        */
        function drawBullet() {
            ...
            var scX = (target.getWidth() * 0.9);
            var scY = (target.getHeight() * 0.9);
            var startX = (target.getWidth() * 0.05);
            var startY = (target.getHeight() * 0.05);
            ...
            for (var i = 0; i < series.length; i++) {
                ...
                ctx.fillText('0', startX, startY + (colHeight * (i + 1)) + 2);
                ...
                if (planned > adjusted) {
                    ...
                    ctx.fillText(formatCurrency(Math.round(planned)), startX + barlength - ((planned + "").length / 2) * 5, startY + (colHeight * (i + 1)) + 2);
                    ctx.beginPath();
                    ctx.moveTo(barStart, startY + (colHeight * i) + 35);
                    ctx.lineTo(startX + barlength, startY + (colHeight * i) + 35);
                    ctx.lineTo(startX + barlength, startY + (colHeight * (i + 1)) - 7);
                    ...
                    if (adjusted > 0) {
                        var adjustedWidth = barlength * adjusted / planned;
                        ctx.beginPath();
                        ctx.moveTo(barStart, startY + (colHeight * i) + 37);
                        ctx.lineTo(barStart + adjustedWidth, startY + (colHeight * i) + 37);
                        ctx.lineTo(barStart + adjustedWidth, startY + (colHeight * (i + 1)) - 9);
                        ctx.lineTo(barStart, startY + (colHeight * (i + 1)) - 9);
                        ctx.lineTo(barStart, startY + (colHeight * i) + 37);
                        ctx.closePath();
                    ...
                    ctx.fillText(formatCurrency(Math.round(adjusted)), startX + barlength - ((adjusted + "").length / 2) * 5, startY + (colHeight * (i + 1)) + 2);
                    ctx.beginPath();
                    ctx.moveTo(barStart, startY + (colHeight * i) + 35);
                    ctx.lineTo(startX + barlength, startY + (colHeight * i) + 35);
                    ctx.lineTo(startX + barlength, startY + (colHeight * (i + 1)) - 7);
                    ...
                    if (planned > 0) {
                        var plannedWidth = barlength * planned / adjusted;
                        ctx.beginPath();
                        ctx.moveTo(barStart, startY + (colHeight * i) + 37);
                        ctx.lineTo(barStart + plannedWidth, startY + (colHeight * i) + 37);
                        ctx.lineTo(barStart + plannedWidth, startY + (colHeight * (i + 1)) - 9);
                        ctx.lineTo(barStart, startY + (colHeight * (i + 1)) - 9);
                        ctx.lineTo(barStart, startY + (colHeight * i) + 37);
                        ctx.closePath();
                        ctx.fillStyle = plannedColor;
                ...
                if (invoiced > 0) {
                    var invoicedWidth = barlength * invoiced / max;
                    ctx.beginPath();
                    ctx.moveTo(barStart, startY + (colHeight * i) + 40);
                    ctx.lineTo(barStart + invoicedWidth, startY + (colHeight * i) + 40);
                    ctx.lineTo(barStart + invoicedWidth, startY + (colHeight * (i + 1)) - 12);
                ...
                if (commited > 0) {
                    var commitedWidth = barlength * commited / max;
                    ctx.beginPath();

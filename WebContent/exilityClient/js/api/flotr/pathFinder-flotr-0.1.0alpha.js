...
// May 09 2012 : Chart Extension - Pathfinder : Aravinda
var typeOfChart = { DEFAULT: 'DEFAULT', PIE: 'PIE', SPEEDOMETER: 'SPEEDOMETER', RADAR: 'RADAR', BARANDLINE: 'BARANDLINE', BAR: 'BAR', SCATTER: 'SCATTER', BUBBLE: 'BUBBLE', HORIZONTALBAR: 'HORIZONTALBAR', STACKED: 'STACKED', HORIZONTALSTACKED: 'HORIZONTALSTACKED', RUNCHART: 'RUNCHART', BULLET: 'BULLET', SUNBURST: 'SUNBURST', SANKEY: 'SANKEY' };

// Feb 26 2009 : Y-Axis Label Formatter for Charts - Nirmanith (Start) : Aravinda
var typeOfFormat = new Array();
var typeOfDecimalSeparator = new Array();
var typeOfFieldSeparator = new Array();
var numberOfPlacesForSeparator = new Array();
    ...
    */
    ...
    function Plot(container, data, opts, chartType)
    {
        ...
        */
        ...
        constructCanvas();
        if ((typeOfChart.DEFAULT == chartType) || (typeOfChart.BAR == chartType) || (typeOfChart.SCATTER == chartType) || (typeOfChart.BUBBLE == chartType) || (typeOfChart.HORIZONTALBAR == chartType) || (typeOfChart.STACKED == chartType) || (typeOfChart.HORIZONTALSTACKED == chartType) || (typeOfChart.RUNCHART == chartType))
        {
            bindEvents();
            findDataRanges();
        ...
        */
        ...
        {
            canvasWidth = target.getWidth() || 400;
            canvasHeight = target.getHeight() || 400;
            target.innerHTML = '';
        ...
        */
        function calculateRange(axis, axisOptions ,isXaxis)//modified by Anand..Passing isXaxis parameter so that to set range of bar
        {
            ...
            */
            ...
            if(isXaxis == 'Y' && typeOfChart.HORIZONTALBAR == currentChartType)
            	 axis.max = max + axis.tickSize;
            else
            	 axis.max = max;
        ...
        */
        function calculateSpacing()
        {
            ...
            for (var i = 0; i < yaxis.ticks.length; ++i)
            {
                ...
                //added by anand for truncating the labels
                if(yaxis.ticks[i].label.length>17)
                	{
                		yaxis.ticks[i].label = yaxis.ticks[i].label.substring(0, 14) + '...';
                	}
               //end of addition
            }
            ...
            */
            ...
            labelMaxWidth=100;//added by anand so that lebles width is fixed and it will not shift the bars to the right when lebles width are more than 100
           ...
           //added by anand to remove the horizontal scroll coming for bars when there are nor record in the grid
        ...
        function drawLabels()
        {
            ...
            */
            for (var j = 0, tick = null; j < xaxis.ticks.length; ++j)
            {
                ...
                //modified By Anand to check whether the x-axis labels are number or string 
                if(isNumeric(tick.label) == true)
                	html += '<div style="position:absolute;top:' + (plotOffset.top + plotHeight + options.grid.labelMargin) + 'px;left:' + (plotOffset.left + tHoz(tick.v) - xBoxWidth / 2) + 'px;width:' + xBoxWidth + 'px;text-align:center" class="flotr-grid-label">' + formatCurrency(tick.label) + "</div>";
                else
            ...
            */
            for (var k = 0, tick = null; k < yaxis.ticks.length; ++k)
            {
                ...
                //modified by Anand to get the labels for Y-Axis without  truncting when tick.label.length=1
                if(tick.label.length>1) 
                ...
                if(tick.label.length==1)
                	html += '<div style="position:absolute;top:' + (plotOffset.top + tVert(tick.v) - labelMaxHeight / 2 + 2) + 'px;left:0;width:' + labelMaxWidth + 'px;text-align:right" class="flotr-grid-label">' + tick.label + "</div>";
                //end of modification
            }
            html += '</div>';
            target.insert(html);
        ...
        * 		void
        */
        function drawSeries(series, currentindex, serieslength)
        {
            if (series.lines.show || (!series.bars.show && !series.points.show))
                drawSeriesLines(series, currentindex, serieslength);
            if (series.bars.show)
                drawSeriesBars(series, currentindex, serieslength);
            // Mar 25 2010 : Bug 1004 - Bubble Chart feature - Exility App : Aravinda
            if ((series.points.show) && (typeOfChart.BUBBLE == chartType))
                drawSeriesBubbles(series, currentindex, serieslength);
            else if (series.points.show)
            {
                drawSeriesPoints(series, currentindex, serieslength);
        ...
        */
        function drawSeriesBars(series, currentindex, serieslength)
        {
            function plotBars(data, barWidth, offset, fill)
            {
                ...
                {
                	var x = data[i][0], y = data[i][1];
                	var barLabel = data[i][3];//added by Anand on 3-Jan-2012 for horizontalbar
                    ...
                    if (typeOfChart.HORIZONTALBAR == currentChartType)
                    {
                        ...
                        top = y - (1 / 2) + 1 * (((curIndex - 1) > 0) ? (curIndex - 1) : 0);
                        bottom = top + 1; 
                       
                        ctx.fillStyle = '#6699FF'; // added by anand
                        
                        /*added by vinay on 02-jan-2013
                         * pushing data of each bar in to bars array in chartData object
                         */
                         if(i != 0)
                        {
                        	var bar = { left: tHoz(left) , right: tHoz(right) ,bottom: tVert(bottom), top: tVert(top), idx: i, helpText: data[i][2] || ('help line-' + chartData.bars.length + ' bar ' + i) };
                    ...
                    } else if (typeOfChart.STACKED == currentChartType)
                    {
                        ...
                        top = bottom + y;
                        /*added by vinay for gradience in stacked chart on 14th-dec-2012*/
                        /*Commenting as per Vinay's Suggestion for colors not coming in Horizontal stacked chart*/
//                        if (series.bars.fill)
//                        {
//                            var grad = ctx.createLinearGradient(tHoz(left), tVert(top), tHoz(right), tVert(top));
//                            var clr = parseColor(series.color);
//                            clr.scale(0.3, 0.3, 0.3);
//                            grad.addColorStop(1, clr.toString());
//                            clr.scale(4, 4, 4);
//                            grad.addColorStop(0.5, clr.toString());
//                            clr.scale(0.3, 0.3, 0.3);
...
//                        }
                    ...
                    */
                    if (fill)
                    {
                    	//var barBorder = 1.5;
                    	var xleft = tHoz(left);
                    	var xright = tHoz(right);
                    	var ybottom = tVert(bottom);
                    	var ytop = tVert(top);
                    	var midPoint = ytop - (ytop - ybottom)/2;
                    	ybottom = midPoint - 7;
                    	ytop = midPoint + 7;
                        ctx.beginPath();
                        ...
                        drawTextOnBars(left,right,top,bottom,i,x,y,barLabel,currentindex,serieslength);//added by vinay for labels on stacked chart..modified by Anand on 3-jan-2012
                    }
                    ...
                    */
                    if (drawLeft || drawRight || drawTop)
                    {
                    	...
                    	ytop = midPoint + 7;
                        ctx.beginPath();
                        ctx.moveTo(tHoz(left), (ybottom) + offset);
                        if (drawLeft) ctx.lineTo(tHoz(left), (ytop) + offset);
                        else ctx.moveTo(tHoz(left), (ytop) + offset);
                        if (drawTop) ctx.lineTo(tHoz(right), (ytop) + offset);
                        else ctx.moveTo(tHoz(right), (ytop) + offset);
                        if (drawRight) ctx.lineTo(tHoz(right), (ybottom) + offset);
                        else ctx.moveTo(tHoz(right), (ybottom) + offset);
                        ctx.closePath();
                        ctx.stroke();
                    }
                }
        ...
        /*added by vinay for labels on stacked,horizontalbar chart*/
        function drawTextOnBars(left, right, top, bottom, i, x, y,barLabel,j, totalStacks)//modified by anand on 3-jan-2012..added barLabel
        { 
        	if (i == 0) return;
        	var ytop = tVert(top);
        	var yBottom = tVert(bottom);
        	...
        	if (typeOfChart.STACKED == currentChartType)
        	{
        		var previousTop = prvTop[i] || 0;
        		var midY = previousTop + (top - previousTop) / 2;
        		var midH = xleft + (xright - xleft) / 4;

        		ctx.fillStyle = 'black'; /* count label color*/
        		if ((yBottom - ytop) >= 15)
        			ctx.fillText(top - previousTop, midH, tVert(midY));/* write the count on each stack */

        		prvTop[i] = top;

        		if (j == totalStacks - 1)/* write the count on the topest stack */
        			ctx.fillText(top, midH, ytop - 5);
        	}

        	if (typeOfChart.HORIZONTALBAR == currentChartType)
        	{
        		var previousTop = bottom;
        		var midY = previousTop + (top - previousTop) / 2;

        		ctx.fillStyle = 'black'; /* count label color*/
        		prvTop[i] = top;
        		
        		//modified by anand for horizontal bar..
        		if (j == totalStacks - 1)/* write the count on the topest bar */
        			{
        				if(barLabel == x)
        				 ctx.fillText(x, xright + 3, tVert(midY));
        				if(barLabel != x)
        				ctx.fillText(barLabel, xright + 3, tVert(midY));
        			}
        ...
        */
        function mouseMoveHandler(event,eventName)
        {
            ...
            var offset = overlay.cumulativeOffset();
            var chartScroll = document.getElementById('scrollPanel').scrollLeft || 0; //for horizontal scroll position of charts in pathfinder
            if ('BULLET' == chartType) 
            {
            ...
            {    	
                ...
                if(event.pageX < docXY.x || event.pageX > (docXY.x +canvasWidth) || event.pageY < offset.top || event.pageY > (offset.top + canvasHeight))
            	{
            ...
            if ('HORIZONTALBAR' === chartType)
            {
            	if(px < 0)
            	 ...
            	 {
            		 var bar = bars[barIdx];
            		 if (px < bar.left) //mouse is before this point. mouse is not on any bar
            			 break;

            		 if (px > bar.left  && px < bar.right && py > bar.bottom && py < bar.top)
            		 {
            			 barFocussed = bar;
            			 break;
            		 } 
            	 ...
            	 if (!chartData.lastBar && !barFocussed) //floating around...
            		 return;

            	 if (chartData.lastBar && chartData.lastBar === barFocussed)
            	 {
                 ...
                 //save this bar for optimization.
                 chartData.lastBar = barFocussed;
                 return;         	 
            }
            if ('DEFAULT' === chartType)
            {
                if(px < 0)
                    px += chartScroll; 
            ...
            if ('PIE' === chartType || 'SUNBURST' === chartType)
            {
            try{
                ...
                if (('PIE' === chartType || 'SUNBURST' === chartType) && highlightRow)
                {
                    if (rowIdx != null)
                    {
...
//                    for (var i = 0; i < n; i++) {
//
//                        pieX = pieData.originX;
//                        pieY = pieData.originY;
//                        pieR = pieData.radius;
//                        startAngle = -Math.PI * 0.5;
//                        if (i > 0) startAngle = pieData.endAngles[i - 1];
//                        endAngle = pieData.endAngles[i];
//                        percent = pieData.piePercent.percentage[i];
//                        percentX = pieData.piePercent.percentX[i];
//                        percentY = pieData.piePercent.percentY[i];
//
//                        if (angle <= endAngle) {
//
//                            octx.clearRect(0, 0, canvasWidth, canvasHeight);
...
//                		pos.rowIdx = -1;
//
//            		if (typeof pieData.lastIdx == 'number' && pieData.lastIdx == rowIdx) 
//            			return;
//            		else
//            			target.fire('flotr:mousemove', [event, pos]);
//
...
//        }
        ...
        */
        function drawPie()
        {
            ...
            //set min-max for x-y
            xaxis.min = 0;
            var wd = xaxis.max = target.getWidth();
            yaxis.min = 0;
            var ht = yaxis.max = target.getHeight();
            hozScale = 1;
            vertScale = 1;
            ...
            for (var i = 0; i < nbr; i++)
            {
                ...
                grad.addColorStop(1, clr.toString());
                //grad.addColorStop(0.4, clr.toString());
                // grad.addColorStop(0.6, clr.toString());
                clr.scale(2.5, 2.5, 2.5);
                grad.addColorStop(0, clr.toString());
                ctx.fillStyle = grad;  //we have trouble getting the right gradient....
                // ctx.fillStyle = pieSeriesColors[i];
                ctx.fill();

                /************************ temp fix for label wrapping
                //do we need to print % ?
                if (showValue)
                {
                ctx.save();
                ...
                var y = pcY + Math.sin(angle) * pRadius / 2;
                y = Math.round(y);
        ...
        */
        function drawPieLegends(labels, data, colors)
        {
            ...
            for (var i = 0; i < labels.length; ++i)
            {
                ...
                fragments.push(div);
                fragments.push('</td><td class="flotr-legend-label">');
        ...
        function drawSunBurstOld()
        {
            ...
            for (var i = 0; i < pieSeries.length; i++)
            {
                ...
                if (Math.abs(startAngle - endAngle) > 0.15)
                {
                    ...
                    if (pieSeries[i][1].length * 5 > availableWidth)
                    {
                        ctx.fillText(pieSeries[i][1].substring(0, Math.floor(availableWidth / 5)), 0, 0);
                    } else
                    {
                        ctx.fillText(pieSeries[i][1], 0, 0);
                    }
                    ctx.restore();
                }
                ...
                if (i == (pieSeries.length - 1))
                {
                    ...
                    if (pieSeries[i][3].length * 5 > availableWidth)
                    {
                        ctx.fillText(pieSeries[i][3].substring(0, Math.floor(availableWidth / 5)), pcX - (pieSeries[i][3].length / 2) * 5, pcY + 3);
                    } else
                    {
                        ctx.fillText(pieSeries[i][3], pcX - (pieSeries[i][3].length / 2) * 5, pcY + 3);
                    }
                    ctx.globalAlpha = 1;
                }
        ...
        */
        function drawSunBurst()
        {
            ...
            var pcX = (wd / 2);
            var pcY = (ht / 2);
            var pRadius = (pcX > pcY) ? (pcY * 0.95) : (pcX * 0.95);
            ...
            for (var i = 0; i < nbr; i++)
            {
                ...
                pieSeriesColors[i]=(groupIndex < options.colors.length) ? parseColor(options.colors[groupIndex]) : getColor(true);
            }
            ...
            while (node) //better than for loop because of the way we are handling group change
            {
                ...
                if (!node || group != node.group)
                {
                    ...
                    else //we are done. draw the core circle
                    {
                        ...
                        drawSlice(groupSlice, 0, true);
                        drawCoreImage(groupSlice);
                    }
                }
                startAngle = endAngle;
            }
            pieData = { originX: pcX, originY: pcY, radius: pRadius, groupRadius: groupRadius, coreRadius: coreRadius, endAngles: endAngles, colors : pieSeriesColors, lastIdx: null};
        ...
        * 		void
        */
        function drawSlice(slice, gradientStart,isCore)
        {
            ctx.globalAlpha = 1;
            ctx.beginPath();
            ...
            if (!slice.label || Math.abs(angle) <= 0.2) // changed from 0.1 to 0.2 as labels are overlapped to other slice-- Anand (7th July 2013)
                return;
            ctx.save();
        ...
        */
        function drawSankey()
        {
            ...
            //set min-max for x-y
            xaxis.min = 0;
            var w = xaxis.max = sankeyData.maxX = target.getWidth();
            yaxis.min = 0;
            var h = yaxis.max = sankeyData.maxY = target.getHeight();
            hozScale = 1;
            vertScale = 1;

            //our actual chart area: 5% top/bottom margin. 20% on either side for labels
            sankeyData.leftLabelWidth = Math.round(0.17 * w) - 3; //1 pixel for borders 2 pixel for clearance on left/right..
            sankeyData.rightLabelWidth = Math.round(0.2 * w) - 3;
            sankeyData.startX = Math.round(0.17 * w);
            sankeyData.endX = Math.round(0.783 * w);
            sankeyData.startY = Math.round(0.005 * h);//vertical scale is modified by Anand as height is set dynamically from page and with eariler scale I am getting enough margin from top.
            sankeyData.endY = Math.round(0.99 * h);
            ...
            sankeyData.leftScale = (0.99 * h - sankeyData.leftLabels.length * 10) / sankeyData.value;
            sankeyData.rightScale = (0.99 * h - sankeyData.rightLabels.length * 10) / sankeyData.value;
        ...
        */
        ...
        function sankeyDrawLabels()
        {
            ...
            div.style.top = '0px';
            var t = [];
            sankeyLabelWorker(sankeyData.leftLabels, sankeyData.leftBars, 'left', 2, t);
        ...
        function sankeyLabelWorker(labels, bars, side, left, t)
        {
            ...
            for (var i = 0; i < labels.length; i++)
            {
                ...
                if(side == 'left')
                t.push(sankeyData.leftLabelWidth);
                if(side == 'right')
                t.push(sankeyData.rightLabelWidth);
                t.push('px;top:');
                t.push(top);
                t.push('px;left:');
                t.push(left);
                ...
                t.push(hite);
                t.push('px"><span ');
                if (label.length > 20)//Changed by Hemanth: Capital Leters occupies more space.
                {
                    t.push('title="');
                    t.push(label);
                    t.push('"');
                    label = label.substring(0, 20) + '...';//Changed by Hemanth: Capital Leters occupies more space.
                }
                t.push('>');
        ...
        * 		void
        */
        /**in drawBullet function formatter is added to check whehter the values should be req to make dollar formatted
         * or not--Anand 12-aug-2013*/
        function drawBullet()
        {
            ...
            var scX = (target.getWidth() * 0.95); // change the multiper from 0.9 to 0.95 so that bar length should increase further..Anand
            var scY = (target.getHeight() * 0.9);
            var startX = (target.getWidth() * 0.05);
            var startY = (target.getHeight() * 0.005);//modified By Anand on 3-Jan-2012 to change the starting point of bullet chart so as to reduce the margin at the top as grid height is dynamic.
            var colHeight = scY / series.length;
            colHeight = 70; //50;
            var barStart = startX + 2;
            var barlength = (scX - (startX + 2));
            for (var i = 0; i < series.length; i++)
            {
            	
                ctx.fillText(series[i].label, startX + 2, startY + (colHeight * i) + 30);
               if(options.formatter =='usd')
               		ctx.fillText('$0', startX+2, startY + (colHeight * (i + 1)) + 2);//Modified by Anand..0 is formated
               else 
                   ctx.fillText('0', startX+2, startY + (colHeight * (i + 1)) + 2);//Modified by Anand..0 is formated

                var invoiced = series[i].data[0];
                var invoicedColor = (0 < options.colors.length) ? options.colors[0] : getColor();
                var commited = series[i].data[1];
                var commitedColor = (1 < options.colors.length) ? options.colors[1] : getColor();
                var planned = series[i].data[2];
                var plannedColor = (2 < options.colors.length) ? options.colors[2] : getColor();
                var adjusted = series[i].data[3];
                var adjustedColor = (3 < options.colors.length) ? options.colors[3] : getColor();
                var toolTip = series[i].data[4] || ('help-text of bullet'+ i);
                var max,plannedWidth, adjustedWidth,invoicedWidth,commitedWidth,barLabel;
                
                if(planned >= adjusted && planned >= commited && planned >=invoiced)
                	{
                		max= planned;
                		barLabel = 'Planned: ';
                	}
                else if (adjusted > planned && adjusted >= commited && adjusted >= invoiced)
                	{
                		max= adjusted;
                		barLabel = 'Adjusted: ';
                	}
                else if (commited > planned && commited > adjusted && commited >= invoiced)
                	{	
                		max= commited;
                		barLabel = 'Committed: ';
                	}
                else if (invoiced > planned && invoiced > commited && invoiced >adjusted)
                	{
                		max= invoiced;
                		barLabel = 'Invoiced: ';
                	}
                
                var labelLength = ((max + "").length) + ((barLabel + "").length);
                
                var labelWidth = labelLength * 6.7;
                
                if(options.formatter =='usd')
            	{
		            if ((max + "").length == 1)
		            	ctx.fillText(barLabel + '$'+formatCurrency(Math.round(max)),  startX + barlength - labelWidth + 5, startY + (colHeight * (i + 1)) + 2); 
		            else
		            	{
		            		//ctx.fillText(barLabel + '$'+formatCurrency(Math.round(max)),  startX+barlength- ((max + "").length)*8-10, startY + (colHeight * (i + 1)) + 2);
		            	ctx.fillText(barLabel + '$'+formatCurrency(Math.round(max)), startX + barlength - labelWidth , startY + (colHeight * (i + 1)) + 2);
		            	}
		        }
		        else
		          {
		             if ((max + "").length == 1)
		               ctx.fillText(Math.round(max),  startX+barlength - ((max + "").length) * 7, startY + (colHeight * (i + 1)) + 2); 
		             else
		               ctx.fillText(Math.round(max),  startX+barlength - ((max + "").length) * 7, startY + (colHeight * (i + 1)) + 2);
                } 
                
                var bullet = { label: series[i].label, startX: barStart, endX: barStart + barlength, startY: startY + (colHeight * i) + 35, endY: startY + (colHeight * (i + 1)) - 7, helpText: toolTip };
                chartData.bullets.push(bullet);
                
                if (planned > adjusted)
                {
                 //   max = planned;
                    //commentd below line as middle value is not required and provide currency formated and roud off value for planned amount --anand
                    // ctx.fillText(planned / 2, barStart + barlength / 2 - ((planned / 2 + "").length / 2) * 5, startY + (colHeight * (i + 1)) + 2);
                  
                    /*modified by Anand on 12-Aug-2013
                     * Amount is formated based on the value of Formatter and values are aligned with the start
                     *  and end point of bar i.e. left and right aligned
                     *  end point for value is calculated as per data avaliable
                     * when amount is 0 then length is 1*/
//                    if(options.formatter =='usd')
//                    	{
//                    if ((planned + "").length == 1)
//                        ctx.fillText('Planned: $'+formatCurrency(Math.round(planned)), startX + barlength - ((planned + "").length) * 5 - 56 , startY + (colHeight * (i + 1)) + 2); 
//                    else
//                        ctx.fillText('Planned: $'+formatCurrency(Math.round(planned)), startX + barlength - ((planned + "").length) * 8 - 52, startY + (colHeight * (i + 1)) + 2); 
//                    	}
//                    else
//                    	{
//                    	if ((planned + "").length == 1)
//                            ctx.fillText(Math.round(planned), startX + barlength - ((planned + "").length) * 5, startY + (colHeight * (i + 1)) + 2); 
//                        else
//                            ctx.fillText(Math.round(planned), startX + barlength - ((planned + "").length) * 7+3, startY + (colHeight * (i + 1)) + 2); 
//                    	}
                    ctx.beginPath();
                    plannedWidth = barlength * planned / max;
                    ctx.moveTo(barStart, startY + (colHeight * i) + 35);
                    ctx.lineTo(barStart + plannedWidth, startY + (colHeight * i) + 35);
                    ctx.lineTo(barStart + plannedWidth, startY + (colHeight * (i + 1)) - 7);
                    ctx.lineTo(barStart, startY + (colHeight * (i + 1)) - 7);
                    ctx.lineTo(barStart, startY + (colHeight * i) + 35);
                    ctx.closePath();
                    ctx.fillStyle = plannedColor;
                    ctx.fill();
                    		
                    //bar start point and end point is adjusted by Anand as per requirement
                    
                    if (adjusted > 0)
                    {
                        adjustedWidth = barlength * adjusted / planned;
                        ctx.beginPath();
                        ctx.moveTo(barStart, startY + (colHeight * i) + 40);
                        ctx.lineTo(barStart + adjustedWidth, startY + (colHeight * i) + 40);
                        ctx.lineTo(barStart + adjustedWidth, startY + (colHeight * (i + 1)) - 12);
                        ctx.lineTo(barStart, startY + (colHeight * (i + 1)) - 12);
                        ctx.lineTo(barStart, startY + (colHeight * i) + 40);
                        ctx.closePath();
                ...
                {
                    //max = adjusted;
                    //commentd below line as middle value is not required and provide currency formated and roud off value for adjusted amount --anand
                    //ctx.fillText(adjusted / 2, (startX + 2) + barlength / 2 - ((adjusted / 2 + "").length / 2) * 5, startY + (colHeight * (i + 1)) + 2);
                   //alert('startX:'+startX+'\nbarlength:'+barlength+'(adjusted + "").length :'+(adjusted + "").length);
                    
                   /*modified by Anand on 12-Aug-2013
                     * Amount is formated and values are aligned with the start and end point of bar i.e. left and right aligned
                     * when amount is 0 then length is 1*/
//                    if(options.formatter =='usd')
//                    	{
//                    if ((adjusted + "").length == 1)
//                    ctx.fillText('Adjusted: $'+formatCurrency(Math.round(adjusted)),  startX+barlength - ((adjusted + "").length) * 5 - 60 , startY + (colHeight * (i + 1)) + 2); 
//                    else
//                    ctx.fillText('Adjusted: $'+formatCurrency(Math.round(adjusted)),  startX+barlength - ((adjusted + "").length) * 8 - 56 , startY + (colHeight * (i + 1)) + 2);
//                    	}
//                    else
//                    	{
//                    	 if ((adjusted + "").length == 1)
//                             ctx.fillText(Math.round(adjusted),  startX+barlength - ((adjusted + "").length) * 5 , startY + (colHeight * (i + 1)) + 2); 
//                        else
//                             ctx.fillText(Math.round(adjusted),  startX+barlength - ((adjusted + "").length) * 7 + 3, startY + (colHeight * (i + 1)) + 2);
//                        }
//                    	
                    	
                    ctx.beginPath();
                    adjustedWidth = barlength * adjusted / max;  
                    ctx.moveTo(barStart, startY + (colHeight * i) + 35);
                    ctx.lineTo(barStart + adjustedWidth, startY + (colHeight * i) + 35);
                    ctx.lineTo(barStart + adjustedWidth, startY + (colHeight * (i + 1)) - 7);
                    ctx.lineTo(barStart, startY + (colHeight * (i + 1)) - 7);
                    ctx.lineTo(barStart, startY + (colHeight * i) + 35);
                    ...
                    if (planned > 0)
                    {
                        plannedWidth = barlength * planned / adjusted;
                        ctx.beginPath();
                        ctx.moveTo(barStart, startY + (colHeight * i) + 40);
                        ctx.lineTo(barStart + plannedWidth, startY + (colHeight * i) + 40);
                        ctx.lineTo(barStart + plannedWidth, startY + (colHeight * (i + 1)) - 12);
                        ctx.lineTo(barStart, startY + (colHeight * (i + 1)) - 12);
                        ctx.lineTo(barStart, startY + (colHeight * i) + 40);
...
//For invoices modification is done for setting the strip width(decreased the width) and starting and end point of bar by Anand-  Aug 12th,2013
                if (invoiced > 0)
                {        
                    invoicedWidth = barlength * invoiced / max;
                    ctx.beginPath();
                    ctx.moveTo(barStart, startY + (colHeight * i) + 45);
                    ctx.lineTo(barStart + invoicedWidth, startY + (colHeight * i) + 45);
                    ctx.lineTo(barStart + invoicedWidth, startY + (colHeight * (i + 1)) - 17);
                    ctx.lineTo(barStart, startY + (colHeight * (i + 1)) - 17);
                    ctx.lineTo(barStart, startY + (colHeight * i) + 45);
                    ctx.closePath();
                    ctx.fillStyle = invoicedColor;
                    ctx.fill();
                }

                if (commited > 0)
                {
                    commitedWidth = barlength * commited / max;
                    ctx.beginPath();
                    ctx.moveTo(barStart + commitedWidth, startY + (colHeight * i) + 35);
                    ctx.lineTo(barStart + commitedWidth, startY + (colHeight * (i + 1)) - 7);
                    ctx.closePath();
                    ctx.lineWidth = 2;
                    ctx.strokeStyle = commitedColor;
                    ctx.stroke();
                }
              //added condition by Anand for showing different colors when zero value came for planned,adjusted,committed and invoiced
                if(planned == 0 && adjusted == 0 && commited == 0 && invoiced == 0)
            	{
                	ctx.beginPath();
                    ctx.moveTo(barStart, startY + (colHeight * i) + 35);
                    ctx.lineTo(barStart + barlength, startY + (colHeight * i) + 35);
                    ctx.lineTo(barStart + barlength, startY + (colHeight * (i + 1)) - 7);
                    ctx.lineTo(barStart, startY + (colHeight * (i + 1)) - 7);
                    ctx.lineTo(barStart, startY + (colHeight * i) + 35);
                    ctx.closePath();
              ...
              //end of addition by Anand for showing different colors when zero value came for planned,adjusted,committed and invoiced
                ctx.lineWidth = 2;
                ctx.strokeStyle = 'black';
                ctx.fillStyle = 'black';
    ...
    return {
        ...
        draw: function (target, data, options, chartType)
        {
            var plot = new Plot(target, data, options, chartType);
            return plot;
        }
    }
})();
...
//added by anand for getting value as currency formatted
...
//added by Anand to check whether the value passed is number or string
function isNumeric(n) 
{
	  return !isNaN(parseFloat(n)) && isFinite(n);
}
//end of addition for numeric check
